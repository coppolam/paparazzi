 ...
 */
/**
 * @file "modules/decawave/quadrilateration.c"
 * @author Thomas Fijen
 *
 * This code implements a multilateration algorithm using a 
 * nonlinear least Squares approach for 4 anchors. 
 * Note for this module to work, the A matrix must be non-singular.
 * To achieve this, the anchors should not all lie on the same z plane
 * (i.e., they should not all lie at the same height from the ground).
 *
 */

#include "quadrilateration.h"
#include "math/pprz_algebra_float.h"

#define TOL 0.01     // Tolerance 
#define MAXITT 20    // Maximum number of iterations
...
/*
 * This performs Gauss Newton optimisation to find the solution
 * to the nonlinear least squares problem
 */
int nonLinLS_compute(struct Anchor *anchors, struct EnuCoor_f *pos, struct EnuCoor_f *oldPos)
{
  ...
  /*** Outlier rejection ***/
  float error = sqrtf((aveX[count_MAF - 1] - x_hat[0]) * (aveX[count_MAF - 1] - x_hat[0]) +
                      (aveY[count_MAF - 1] - x_hat[1]) * (aveY[count_MAF - 1] - x_hat[1]));
  if (error > 10) { // If the error changes by more than 10 meters we reject the measurement
    if (count_MAF == 0) {
...
{
  float out;
  out = matIn[0] * (matIn[4] * matIn[8] - matIn[5] * matIn[7]) - matIn[1] * (matIn[3] * matIn[8] - matIn[5] * matIn[6]) +
...
{
  for (uint8_t i = 0; i < 3; i++)    {
    mat_out[i] = mat_inA[0] * mat_inB[i] + mat_inA[1] * mat_inB[i + 3] + mat_inA[2] * mat_inB[i + 6] + mat_inA[3] *
                 mat_inB[i + 9];
    mat_out[i + 3] = mat_inA[4] * mat_inB[i] + mat_inA[5] * mat_inB[i + 3] + mat_inA[6] * mat_inB[i + 6] + mat_inA[7] *
                     mat_inB[i + 9];
    mat_out[i + 6] = mat_inA[8] * mat_inB[i] + mat_inA[9] * mat_inB[i + 3] + mat_inA[10] * mat_inB[i + 6] + mat_inA[11] *
                     mat_inB[i + 9];
  }
}
/*
...
{
  for (int i = 0; i < 4; i++)   {
    out[i] = -(X[0] - anchors[i].pos.x) / sqrtf((X[0] - anchors[i].pos.x) * (X[0] - anchors[i].pos.x) +
             (X[1] - anchors[i].pos.y) * (X[1] - anchors[i].pos.y) + (X[2] - anchors[i].pos.z) * (X[2] - anchors[i].pos.z));
    out[i + 4] = -(X[1] - anchors[i].pos.y) / sqrtf((X[0] - anchors[i].pos.x) * (X[0] - anchors[i].pos.x) +
                 (X[1] - anchors[i].pos.y) * (X[1] - anchors[i].pos.y) + (X[2] - anchors[i].pos.z) * (X[2] - anchors[i].pos.z));
    out[i + 8] = -(X[2] - anchors[i].pos.z) / sqrtf((X[0] - anchors[i].pos.x) * (X[0] - anchors[i].pos.x) +
                 (X[1] - anchors[i].pos.y) * (X[1] - anchors[i].pos.y) + (X[2] - anchors[i].pos.z) * (X[2] - anchors[i].pos.z));
  }
}
...
{
  for (int i = 0; i < 4; i++)    {
    F[i] = anchors[i].distance - sqrtf((X[0] - anchors[i].pos.x) * (X[0] - anchors[i].pos.x) + (X[1] - anchors[i].pos.y) *
                                       (X[1] - anchors[i].pos.y) + (X[2] - anchors[i].pos.z) * (X[2] - anchors[i].pos.z));
  }
}
